...
REPO_NAME="jsonpls"
BINARY="jsonpls"
INSTALL_DIR="${JSONPLS_INSTALL_DIR:-$HOME/.jsonpls/bin}"
VERSION="${JSONPLS_VERSION:-}"
...
echo -e "  ${RED} ▀▄▀  ▀▀▄ ▀▄▀ █ ▀█ █▀  █▄▄ ▀▀▄ ${NC}"
echo -e "${NC}"
echo -e "  ${DIM}CSV / XLSX -> AI-friendly JSON${NC}"
...
if [[ "$PATH_UPDATED" == "false" ]]; then
    ...
    case "$CURRENT_SHELL" in
        ...
        fish)
            ...
            echo "set -gx PATH \"$INSTALL_DIR\" \$PATH" > "$HOME/.config/fish/conf.d/jsonpls.fish"
            info "Added to ~/.config/fish/conf.d/jsonpls.fish"
            PATH_UPDATED=true
            ;;
    esac
...
echo ""
echo -e "  ${GREEN}${BOLD}jsonpls installed successfully${NC}"
echo ""

if ! command -v jsonpls &>/dev/null; then
...
echo ""
echo -e "    ${CYAN}jsonpls report.csv${NC}            Convert one file"
echo -e "    ${CYAN}jsonpls *.csv -o ~/data/${NC}      Batch convert"
echo -e "    ${CYAN}jsonpls book.xlsx --jsonl${NC}     XLSX to newline-delimited JSON"
echo -e "    ${CYAN}cat data.csv | jsonpls -${NC}      Read stdin, write stdout"
echo ""
